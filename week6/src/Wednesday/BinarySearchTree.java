...
public class BinarySearchTree {
	...
	private boolean removeInternal(int value, Node root) {
		...
		/*
		 * Node parent = searchParent(value, root); Node target =
		 * searchInternal(value, root);
		 * 
		 * boolean isLeft = false;
		 * 
		 * if( parent.mLeft.mValue == value ) { isLeft = true; }
		 * 
		 * if(isLeaf(target)) { if(isLeft) { parent.mLeft = null; } else {
		 * parent.mRight = null; } } else if (hasBothParents(target)) {
	...
	/*
	 * private void removeLeaf(int value) { Node parent = searchParent(value,
	 * root); boolean isLeft = false;
	 * 
	 * if( parent.mLeft.mValue == value ) { isLeft = true; }
	 * 
	 * if(isLeft) { parent.mLeft = null; } else { parent.mRight = null; } }
	 */
	...
	public void traverse(Node root) { // Each child of a tree is a root of its
										// subtree.
	...
	/*
	 * private void walk(Node node) { Node temp = node; if (temp != null) {
	 * System.out.println(temp.mValue); walk(temp.getLeft());
	 * walk(temp.getRight()); } } private void walk2(Node node) {
	 * printKids(node); walk2(node.mLeft); walk2(node.mRight);
	 * 
	 * }
	 * 
	 * 
	 * private void printKids(Node node) { if (node != null) { if (node.mLeft ==
	 * null && node.mRight == null) { return; } else if (node.mLeft == null) {
	 * System.out.println(node.mRight.mValue); } else if (node.mRight == null) {
	 * System.out.println(node.mLeft.mValue); } } return; }
	 */
