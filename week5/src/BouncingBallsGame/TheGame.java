...
public class TheGame extends JFrame {
	...
	private boolean isPlayerHit() {
		...
		for (Ball ball : mGame.getBalls()) {
			...
			if (ballLowestPoint >= playerHighestPoint && ballLeftmostPoint <= playerRightmostPoint
	...
	private boolean isBallHit() {
		...
		for (Ball ball : mGame.getBalls()) {
			...
			if (ballLowestPoint >= mGame.getLaser().getyAxis() && ballLeftmostPoint <= mGame.getLaser().getxAxis()
	...
	class MyPanel extends JPanel {
		...
		private void drawLaser(Graphics g, LaserBeam laser) {
			...
			g.drawLine((int) laser.getxAxis(), (int) mGame.getPlayer().getyAxis() - 2, (int) laser.getxAxis(),
					(int) laser.getyAxis());
	 ...
	 * System.out.println("Time's up!"); timer.cancel(); //Not necessary because
	 * we call System.exit //System.exit(0); //Stops the AWT thread (and
	 * everything else) } }
	 */
}